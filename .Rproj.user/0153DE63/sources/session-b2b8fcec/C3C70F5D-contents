...
for(i in 1:length(data_folder)){
  list_data <- list.files(paste0(here(), "/01 Raw data/", data_folder[i]))
  map(list_data, ~{
    import_data(paste0(here(), "/01 Raw data/", data_folder[i], "/", .x)) %>%
      ## save this in global environment using assign() function and name it as the file name, but delete suffix (.csv, .rds, and so on)
      assign(str_to_lower(str_replace_all(gsub("\\..*", "", .x), "-", "_")), ., envir = globalenv())
    })
}
