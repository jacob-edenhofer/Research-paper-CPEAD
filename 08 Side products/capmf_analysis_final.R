...
setFixest_etable(postprocess.tex = NULL,
                 ## replace the above argument with fragment_extract if needed
                 fitstat = ~ n + r2,
...
## Notes 
### I can show the link between trade (openc, nv_ind_manf_zs, ne_trd_gnfs_zs) and corporatism (corp_all) quite robustly for stringency at level 2.
### I can show the link between trade (openc, ne_trd_gnfs_zs) and corporatism (corp_all) quite robustly for stringency level 1.
### I can show the link between eco_cip_gross and corporatism (corp_all) quite robustly for adoption level 1. 
...
oecd_stringency_LEV1 <- oecd_stringency_LEV1 %>%
  ...
  mutate(across(
    .cols = c(corporatism, variables, covariates),
...
# prune vector of interaction terms of interest 
vars_pruned <- variables[grepl("openc|ne_trd_gnfs_zs|eco_cip_gross|unhappy", variables)]


# extract fragments and save in one regression table 
customised_list <- list()
for(i in 1:length(vars_pruned)){
  customised_list[[i]] <- feols(as.formula(paste("obs_value1 ~ corp_all_lag1*", paste0(vars_pruned[i], "_lag1"),  "+ csw0(", 
                                            paste(paste0(covariates, "_lag1"), collapse = ", "), ")" ,"| iso3c + half_decade + clim_act_pol")),
                           vcov = ~iso3c,
                           data = oecd_stringency_LEV1)
  etable(customised_list[[i]], 
         title = paste("Exploratory analysis: ", vars_pruned[i]))
}



# map(vars_pruned, ~{
#   customised_list <- feols(as.formula(paste("obs_value1 ~ corp_all_lag1*", paste0(.x, "_lag1"),  "+ csw0(", paste(paste0(covariates, "_lag1"), collapse = ", "), ")" ,"| iso3c + half_decade + clim_act_pol")),
#                            vcov = ~iso3c,
#                            data = oecd_stringency_LEV1)
#   etable(customised_list, 
#          tex = T, 
#          replace = T, 
#          title = paste("Exploratory analysis: ", .x))
# })
